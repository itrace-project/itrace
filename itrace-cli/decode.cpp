...
std::pair<std::optional<struct timespec>, std::optional<struct timespec>> parse_time_input(
    std::string time
) {
	size_t split = time.find(",");
	if (split == std::string ::npos) {
	...
	if (!start_str.empty()) start = parse_time(start_str);
	if (!end_str.empty()) end = parse_time(end_str);
