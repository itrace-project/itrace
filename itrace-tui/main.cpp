...
int main() {
    ...
    std::map<std::string, std::vector<std::string>> args_labels = {
        {"Record", {"Target", "Output", "PID", "Filter Symbol", "Filter Instruction Pointer", "Snapshot"}},
    ...
    Component ui = Renderer(top_container, [&] {
        if (current_screen == ScreenState::Menu) {
            ...
            if (show_output_popup[cmd]) {
                layout = window(text("Command Output"), vbox({
                    text("Command:") | bold,
                    paragraph(full_command[cmd]),
                    separator(),
                    text("Output:") | bold,
                    paragraph(command_output[cmd]),
                    separator(),
    ...
    ui = CatchEvent(ui, [&](Event e) {
        if (current_screen == ScreenState::Menu) {
            if (e == Event::ArrowDown) { menu_selected = (menu_selected + 1) % commands.size(); return true; }
            if (e == Event::ArrowUp) { menu_selected = (menu_selected - 1 + commands.size()) % commands.size(); return true; }
            if (e == Event::Return) { current_screen = ScreenState::ArgsInput; input_containers[commands[menu_selected]]->TakeFocus(); return true; }
            if (e == Event::Escape) { screen.Exit(); return true; }
        } else {
            auto& cmd = commands[menu_selected];
            int& arg_selected = focused_input_index[cmd];

            if (show_info_popup[cmd] || show_output_popup[cmd] || show_completed_output_popup[cmd]) {
                if (e == Event::Tab || e == Event::Escape) { show_info_popup[cmd] = false; show_output_popup[cmd] = false;  show_completed_output_popup[cmd] = false; return true; }
                if (e == Event::Return) {
                ...
                if (e == Event::ArrowDown) { arg_selected = (arg_selected + 1) % input_components[cmd].size(); input_components[cmd][arg_selected]->TakeFocus(); return true; }
                if (e == Event::ArrowUp) { arg_selected = (arg_selected - 1 + input_components[cmd].size()) % input_components[cmd].size(); input_components[cmd][arg_selected]->TakeFocus(); return true; }
                if (e == Event::Tab) { show_info_popup[cmd] = true; return true; }
               ...
               if (e == Event::Return) {
                ...
                if (e == Event::Escape) { current_screen = ScreenState::Menu; return true; }
            }
        }
        return false;
    });
